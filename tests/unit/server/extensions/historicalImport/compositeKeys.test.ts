...
describe('encode / decode', () => {
  it('round-trips a card_label key in the declared column order', () => {
    ...
    expect(decodeCompositeTargetId('card_label', encoded)).toEqual({
      card_id: CARD,
      label_id: LABEL,
    });
  });
  ...
  it('round-trips a card_member key', () => {
    ...
    expect(decodeCompositeTargetId('card_member', encoded)).toEqual({
      card_id: CARD,
      user_id: USER,
    });
  });
  ...
  it('rejects parts that could embed a separator or whitespace', () => {
    ...
    expect(() =>
      encodeCompositeTargetId('card_label', { card_id: 'a b', label_id: LABEL })
    ).toThrow(/must be a non-empty id/);
  ...
  it('rejects encoding for a non-composite entity type', () => {
    expect(() => encodeCompositeTargetId('card', { id: CARD })).toThrow(
      /not a composite-key entity/
    );
  });
