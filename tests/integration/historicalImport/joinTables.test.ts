...
function joinOp(partial: Partial<ImportOperation> & Pick<ImportOperation, 'op_id' | 'entity_type' | 'source_id'>): ImportOperation {
  return {
    operation: 'create',
    target_id: undefined,
    provenance: {
...
describe('join tables — apply', () => {
  ...
  it('refuses to claim a native join row (composite key already exists, no provenance)', async () => {
    const deps = freshDeps();
    deps.rows.set(`card_label:${CARD_TARGET}:${LABEL_ID}`, { card_id: CARD_TARGET, label_id: LABEL_ID });
    const plan = planWithJoins();
    const res = (await applyPlan(plan, await gatesFor(plan, deps), deps, OPERATOR)) as {
  ...
  it('links an existing join row instead of recreating it', async () => {
    const deps = freshDeps();
    deps.rows.set(`card_member:${CARD_TARGET}:${MEMBER_ID}`, { card_id: CARD_TARGET, user_id: MEMBER_ID });
    const plan = planWithJoins();
    ...
    expect(deps.provenance.some((p) => p.target_ref === `card_member:${CARD_TARGET}:${MEMBER_ID}`)).toBe(true);
  });
