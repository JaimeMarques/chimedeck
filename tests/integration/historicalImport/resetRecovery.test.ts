...
describe('reset — provenance-only', () => {
  it('reports the rows it leaves behind (no more silent re-apply failure)', async () => {
    ...
    const res = await resetPlan((await validatePlan(plan, deps, OPERATOR)).plan_hash, deps, OPERATOR);
  ...
  it('records the created targets in the audit log so recovery survives a reset', async () => {
    ...
    const detail = audit.detail as { created_targets?: Array<{ entity_type: string; target_id: string }> };
...
describe('reset — destructive recovery', () => {
  it('is refused unless the server gate is enabled', async () => {
    ...
    const res = await recoverPlan(hash, deps, OPERATOR, { confirmDestructive: true, recoveryEnabled: false });
  ...
  it('is refused without the explicit destructive confirmation', async () => {
    ...
    const res = await recoverPlan(hash, deps, OPERATOR, { confirmDestructive: false, recoveryEnabled: true });
  ...
  it('deletes only the rows the plan created and clears its provenance', async () => {
    ...
    const report = await recoverPlan(hash, deps, OPERATOR, { confirmDestructive: true, recoveryEnabled: true });
  ...
  it('lets the same plan re-apply cleanly after recovery (QA-6 revisited)', async () => {
    ...
    const reapply = (await applyAll(plan, deps)) as { operations_applied: number; operations_blocked: number };
