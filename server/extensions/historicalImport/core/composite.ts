...
export function encodeCompositeTargetId(entityType: string, key: Record<string, string>): string {
  ...
  if (!columns) {
    throw new CompositeKeyError('not-composite-entity', `${entityType} is not a composite-key entity type`);
  }
...
export function decodeCompositeTargetId(entityType: string, targetId: string): Record<string, string> {
  const columns = compositeKeyColumns(entityType);
  if (!columns) {
    throw new CompositeKeyError('not-composite-entity', `${entityType} is not a composite-key entity type`);
  }
