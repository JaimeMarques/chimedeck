...
- Defeito encontrado pelo ensaio, **fora do âmbito destas duas correções** e encaminhado como tarefa própria: um payload staged com `created_at` em entidades cuja tabela não tem essa coluna (ex. `lists`) faz falhar o INSERT (`column "created_at" of relation "lists" does not exist`).
