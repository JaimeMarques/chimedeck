  ...
  "operations": [
    {
      ...
      "operation": "create|link|correct|enrich|correct_card_description",
      "provenance": {
        "source_system": "trello",
        "source_id": "<= source_id",
...
- `correct` só aceita `entity_type=comment`, `target_id`, `payload_ref`, `historical_author` resolvido e a pré-imagem exacta `expected_target_fields={user_id,content,created_at,updated_at,parent_id}`. O adapter bloqueia drift, comentários apagados e claims incompatíveis; actualiza apenas esses campos e grava/verifica provenance na mesma transação. O `content` histórico é escrito **verbatim**, sem `trim`, normalização, escape ou `sanitizeRichText`: o payload SHA-256 fixado é a fonte revista, e o readback tem de comparar a string exacta. Só U+0000 e surrogates UTF-16 não emparelhados são recusados antes de abrir a transação, com erro explícito de não-representabilidade por `PostgreSQL TEXT`; nunca há fallback lossy. Re-run com a pós-imagem exacta é `noop`.
- `correct_card_description` é uma categoria administrativa distinta, limitada a `entity_type=card` e a uma única mutação raw `description`. Exige payload manifest-pinned, pré-imagem exacta, fingerprint da linha completa, hashes raw origem/destino e uma entrada source→target no artefacto privado imutável fixado pelo plano; faz lock/re-read transacional e escreve apenas `cards.description` + provenance. Contrato completo para o re-freeze: [`historical-import-card-description-correction.md`](./historical-import-card-description-correction.md).
- `create` de `comment` usa a mesma regra lossless para `fields.content` e mantém `historical_author`/`created_at`/`updated_at`. Esta excepção é estritamente da via administrativa de importação com payload privado, contido no staging root e SHA-256 verificado; os endpoints normais de comentários continuam a sanitizar input não confiável. A UI tem de renderizar estes valores sem mutar o texto armazenado e com sanitização no momento de renderização.
...
9. **Hashes externos fail-closed** — quando `plan.input_preconditions` fixa payload manifest, identity map, attachment-object manifest, destination row source ou card-description authorization, `validate` recalcula os SHA-256 canónicos dos ficheiros privados configurados; ausente, ilegível, trocado ou divergente invalida o plano. `dry-run` e `apply` reutilizam este validate antes de qualquer mutação; os artefactos exactos validados são os mesmos já carregados para execução.
...
export HISTORICAL_IMPORT_CARD_DESCRIPTION_AUTHORIZATION=/var/lib/chimedeck/card-description-authorization.json # só para correct_card_description
...
2. Gerar plano + payloads staged (fora do âmbito deste PR; o gerador consome o export Trello e escreve `payload_ref` sob o root). Para `card_label`/`card_member`, o gerador emite `target_id` composto (`<card_id>:<label_id>` / `<card_id>:<user_id>`). Para comentários históricos com `content` presente, emite `create` ou `correct` normal — nunca `blocked_target_semantics` por conteúdo não-canónico — e copia a string exactamente para `fields.content`; `correct` conserva a pré-imagem exacta e o fingerprint. Depois de qualquer alteração aos payloads, regenerar `payload-manifest.json` (SHA-256 de cada `payload_ref`), o precondition `payload_manifest.manifest_sha256`, `plan_hash` e o `destination_fingerprint` de validate/dry-run; não reutilizar confirmações do plano anterior.
